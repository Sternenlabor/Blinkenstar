...
- Maintained firmware flash helpers live in `firmware/dist/`.
...
- Use `firmware/dist/flash.sh` on Unix-like hosts or `firmware/dist/flash.bat` on Windows when you need to flash an already-built `.hex` from `firmware/dist/` with fuse programming.
